...
class xpFile
{
     ...
     * @return
     */
    static function dir_walk($directory, $file_func = null, $dir_func = null, $max_level = 30, $level = 0)
    {
     ...
     *
     */
    static function make_path($path, $permissiom = 0775)
    {
        $ps = explode('/', $path);
        $r = array_shift($ps);
        while (count($ps)) {
     ...
     */
    static function file_in_dir($path, $option = array())
    { //array('detail'=>false, 'level'=>0,'path'=>true, include=>'/\.php$/i', excluede='/\.png$/i')
        ...
        foreach ($entry as $f) {
            ...
            if (($option['include'] ?? false) && !preg_match($option['include'], $f)) continue;
            if (($option['exclude'] ?? false) && preg_match($option['exclude'], $f)) continue;
            $file = $f;
            if (($option['path'] ?? false)) $file = $path . '/' . $f;
            if (($option['detail'] ?? false)) $file = array('name' => $file, 'time' => filectime($path . '/' . $f), 'size' => filesize($path . '/' . $f));
            $files[] = $file;
        }
        return $files;
    }
     ...
     * @return  array of files
     */
    static function dir_in_dir($path, $r = false)
    {
        if (!is_dir($path)) return false;
        $entry = scandir($path);
        foreach ($entry as $f) {
     ...
     * @return array
     */
    static function dir_tree($path, $detail = false, $flat = false, $root = '')
    {
        if (!is_dir($path)) return false;
        $entry = scandir($path);
        foreach ($entry as $f) {
            ...
            if (!is_dir($path . '/' . $f)) {
                ...
                if ($detail) {
                    if ($flat) $files[$pp . '/' . $f] = array('name' => $path . '/' . $f, 'time' => filectime($path . '/' . $f), 'size' => filesize($path . '/' . $f));
                    else $files[] = array('name' => $path . '/' . $f, 'time' => filectime($path . '/' . $f), 'size' => filesize($path . '/' . $f));
                } else {
                    if ($flat) $files[$pp][$f] = $path . '/' . $f;
                    else $files[] = $path . '/' . $f;
                }
            } else {
                if ($flat) $files = xpAS::merge($files, self::dir_tree($path . '/' . $f, $detail, $flat, $root));
                else $files[] = array($f => self::dir_tree($path . '/' . $f, $detail, $flat, $root));
            }
        }
        return $files;
    }
     ...
     * @return array
     */
    static function dir_array($directory, $max_level = 30, $level = 0)
    {
        if ($level++ > $max_level) return;
        if ($directory && substr($directory, -1, 1) != '/') $directory .= '/';
    ...
    static function image_out($image, $type = 'jpg')
    {
     ...
     */
    static function name($n, $pi = null)
    {
        $pi = $pi ? $pi : pathinfo($n);
        return $pi['filename'];
    }
     ...
     */
    static function read_line_until($fp, $str)
    {
        $content = '';
     ...
     * @return string contains
     */
    static function read_block_by_tag($fp, $tag)
    {
        $start = "<$tag>";
        $end = "</$tag>";
     ...
     */
    static function upload($fieldname, $path, $new_filename = null)
    {
        if ($_FILES[$fieldname]['name'] != '') {
            self::make_path($path);
            if (is_null($new_filename)) $new_filename = basename($_FILES[$fieldname]['name']);
            if ($new_filename === -1) $new_filename = uniqid();
            if (!move_uploaded_file($_FILES[$fieldname]['tmp_name'], $path . '/' . $new_filename)) return false;
            @chmod($path . '/' . $new_filename, 0777); //
            return $new_filename;
        }
        return false;
    }
     ...
     */
    static function uploadi($i, $fieldname, $path, $new_filename = null)
    {
        if ($_FILES[$fieldname]['name'][$i] != '') {
            self::make_path($path);
            if (is_null($new_filename)) $new_filename = basename($_FILES[$fieldname]['name'][$i]);
            if ($new_filename === -1) $new_filename = uniqid();
            if (!move_uploaded_file($_FILES[$fieldname]['tmp_name'][$i], $path . '/' . $new_filename)) return false;
            @chmod($path . '/' . $new_filename, 0777); //
            return $new_filename;
        }
        return false;
     ...
     */
    static function read_upload($fieldname, $i = null)
    {
     ...
     */
    static function uploads($fieldname, $path, $new_filename = null)
    {
        ...
        foreach ($_FILES[$fieldname]['name'] as $k => $v) {
            ...
            if (move_uploaded_file($_FILES[$fieldname]['tmp_name'][$k], $path . '/' . $nf[$k])) chmod($path . '/' . $new_filename, 0777);
            else $nf[$k] = 'error';
        }
        return $nf;
    }
     ...
     * @return  boolen
     */
    static function test_image_size($fieldname, $width, $height)
    {
     ...
     * @return image
     */
    static function image_resize($image, $nx = null, $ny = null, $type = 'crop', $autorotate = false, $background = '#ffffff', $file_dest = '', $QUALITY = 100)
    {
        if (is_string($image)) {
            $image_delete = 1;
     ...
     * @return image image
     */
    static function image_get($filename)
    {
        return imagecreatefromstring(file_get_contents($filename));
    }
    static function show($img, $type = 'jpg')
    {
        switch ($type) {
            default:
            case 'jpg':
    ...
    static function image_put($img, $filename, $type = 'jpg')
    {
    ...
    static function rotate($src, $angle, $dest = null)
    {
    ...
    static function captcha($setting = array())
    {
        ...
        for ($i = 0; $i < $tx; $i++) {
            ...
            for ($j = 0; $j < $ty; $j++) {
                ...
                $matrix[$i][$j] = $code_matrix[$i_code][$j - $margin][$i_index - $margin];
            }
        }
