...
class xpCaptcha
{
     ...
     * @param int $expired		: ttl
     * @param array $container	: session container
     * @return boolean
     */
    static function check($code, $expired = 300, $container = false)
    {
        if (!$container) $container = $_SESSION;
        $sn = $container[self::$session_name];
        if ($sn['time'] + $expired < time()) return false;
        if ($sn['name'] != $code) return false;
        return true;
    }
    static function set($arr)
    {
        if ($arr['length']) self::$code_length = min(20, $arr['length']);
        if ($arr['dot']) self::$dot = array('x' => min(20, $arr['dot']['x']), 'y' => min(30, $arr['dot']['y']));
    }
    static function generate($arr = null, $show = true, $container = array())
    {
        if ($arr) self::set($arr);
         ...
         * background
         */
        $x = (self::$code_length) * 5 * (self::$dot['x'] + $padding['x']) - 5 * $padding['x'] + $margin['x'] * 2;
        $y = 7 * self::$dot['y'] + 6 * $padding['y'] + $margin['y'] * 2;
        $im = imagecreate($x, $y);
        imagerectangle($im, 0, 0, $x, $y, $bgcolor = imagecolorallocate($im, 255 - mt_rand(0, 64), 255 - mt_rand(0, 64), 255 - mt_rand(0, 64)));
        ...
        for ($k = 0; $k < self::$code_length; $k++) {
            for ($m = 0; $m < 7; $m++) {
                $d = (str_split(self::$matrix[$code[$k]][$m]));
                foreach ($d as $n => $v) {
                    if ($v) {
                        $x0 = $margin['x'] + $k * 5 * (self::$dot['x'] + $padding['x']) + $n * (self::$dot['x'] + $padding['x']) + mt_rand(-50, 50) / 100 * self::$dot['x'];
                        $y0 = $margin['y'] + $m * (self::$dot['y'] + $padding['y']) + mt_rand(-50, 50) / 200 * self::$dot['y'];
                        imagechar($im, mt_rand(2, 5), (int)$x0, (int)$y0, xpAS::password_generator(1, '&@$=02345689DOCHQPS'), $bdcolor = imagecolorallocate($im, mt_rand(0, 64), mt_rand(0, 64), mt_rand(0, 64)));
                        $x0 = $margin['x'] + $k * 5 * (self::$dot['x'] + $padding['x']) + $n * (self::$dot['x'] + $padding['x']) + mt_rand(-50, 50) / 100 * self::$dot['x'];
                        $y0 = $margin['y'] + $m * (self::$dot['y'] + $padding['y']) + mt_rand(-50, 50) / 200 * self::$dot['y'];
                        if (mt_rand(0, 1)) imagechar($im, mt_rand(2, 5), (int)$x0, (int)$y0, xpAS::password_generator(1, '&@$=02345689DOCHQPS'), $bdcolor = imagecolorallocate($im, mt_rand(200, 255), mt_rand(200, 255), mt_rand(200, 255)));
                    }
                }
            }
        }
    ...
    static function simpleRandString()
    {
        mt_srand((float)microtime() * 1000000);
        $new_string = "";
        while (strlen($new_string) < self::$code_length) $new_string .= mt_rand(0, 9);
        return $new_string;
    }
}
