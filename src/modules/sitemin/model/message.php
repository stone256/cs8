...
class sitemin_model_message
{
    ...
    var $system_sender = array(-1 => 'system', -2 => 'cron',);
    ...
    function send_to($receivers, $msg, $sender = null, $groups = null)
    {
        ...
        //create message;
        $arr = array('from' => $sender, 'message' => $msg,);
        if ($groups) $arr['to_group'] = $groups;
        $mid = xpTable::load($this->message_table)->insert($arr);
        foreach ($rc as $kc => $vc) {
            $arr = array('sitemin_message_id' => $mid, 'sitemin_id' => $vc,);
    ...
    function lead($user_id = null)
    {
        ...
        foreach ((array)$msg as $k => $m) {
            if ($_m = xpTable::load($this->message_table)->get(array('id' => $m['sitemin_message_id'] ?? 0))) {
                if (($_m['from'] ?? false) == -1) {
                    $_m['from'] = 'system';
                } else {
                    $u = _factory('sitemin_model_user')->detail($_m['from'] ?? '');
                    $_m['from'] = $u['email'] ?? base64_decode($u['username'] ?? '');
                }
            } else {
                $_m = ['from' => 'public'];
            }
