...
class sitemin_model_user
{
    ...
    function brief($user_id = null)
    {
        ...
        $r = xpTable::load($this->user_table)->get(array('id' => $u), 'username,email,user_role');
        $l = xpTable::load('sitemin_log')->get(array('user_id' => $u, 'route' => 'logged in'), 'count(*) as c'); //, '-created');
        $r['logins'] = $l['c'];
        return $r;
    }
    /**
    ...
    function get($crr)
    {
        ...
        $u['user_role'] = implode(',', xpAS::get($_r, 'data,*,name'));
        return $u;
    }
    ...
    function search_name($q)
    {
         ...
         * )
         */
        $arr = array('search' => array("username like '%$name%'"), 'limit' => 12,);
    ...
    function search_email($q)
    {
         ...
         * )
         */
        $arr = array('search' => array("email like '%$email%'"), 'limit' => 12,);
        $rs = xpTable::load($this->user_table)->lists($arr);
        return $rs;
    }
    function save($q)
    {
        $arr = xpAS::round_up($q, 'username,email,user_role,status');
        if (!$arr['username']) $msg[] = 'username';
        if (!$arr['email']) $msg[] = 'email';
        if (!($roles = trim($arr['user_role']))) $msg[] = 'user_role';
    ...
    function passwordhashsend($email)
    {
        ...
        $drr = array('title' => 'SITEMIN', 'website' => 'SITEMIN', 'link' => _X_URL . '/sitemin/resetpassword/' . $hash, 'name' => $r['username'],);
        $tpl->sets($drr);
        //mail queuing
        $mrr = array('from' => 'forgot password', 'to' => $r['email'], 'subject' => 'Reset my login request', 'body' => $tpl->html(),);
        //email::send($mrr);
