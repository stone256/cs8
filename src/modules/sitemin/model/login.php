...
class sitemin_model_login extends sitemin_model_user
{
    ...
    function current()
    {
        ...
        $r['user_role'] = $r['user_role'] ?? 'public'; //public .none login
        return $r;
    }
    function login($arr, $login_field = 'email')
    {
        if (($r = parent::login($arr)) && $r['status'] == 'active') {
            $r['user_role'] = 'public,' . $r['user_role'];
            Helper::data_set('sitemin,user', $r);
            if ($arr['keeplogin'] ?? false) Helper::data_set('sitemin,keeplogin', 1);
            return $r;
        }
    }
    function logout()
    {
        Helper::data_set('sitemin,user', null);
    }
    ...
    function forgotpassword($q)
    {
        ...
        if ($r) {
            ...
            $tdata = array('title' => 'Reset Password', 'header' => '', 'footer' => '', 'email' => $r['email'], 'reset_link' => _X_URL . '/admin/resetpassword?id=' . $hash,);
            $arr = array('to' => $r['email'], 'from' => 'admin@' . $_SERVER['SERVER_NAME'], 'subject' => 'Login Reset', 'tpl' => _X_LAYOUT . '/email/resetpassword.tpl.html', 'tpl_data' => $tdata,);
