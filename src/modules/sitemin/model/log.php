...
class sitemin_model_log
{
    ...
    function insert($brr = array())
    {
        ...
        $arr = $brr ? $brr : array('user_id' => $u['id'] ?? 0, 'route' => _url(), 'data' => json_encode(array('user' => $u, 'request' => xpAS::round_off($_REQUEST, 'password'), 'cookie' => $_COOKIE, 'server' => $_SERVER)),);
    ...
    function login24()
    {
        $arr = array('route' => 'logged in', "created >'" . xpDate::last_date(xpDate::timestamp()) . "'",);
    ...
    function top10url($days = 1)
    {
        ...
        $sql = "SELECT route, COUNT(*) AS c FROM {$this->table} WHERE route NOT IN {$this->exclude} AND created >'$date' GROUP BY route ORDER BY c DESC LIMIT 10";
    ...
    {
        $date = date("Y-m-d H:i:s", strtotime("-{$days} days"));
        $sql = "SELECT route, COUNT(*) AS c FROM {$this->table} WHERE route NOT IN {$this->exclude} AND created >'$date' GROUP BY route ORDER BY c DESC LIMIT 5";
        $rs = xpTable::load($this->table)->q($sql);
        $routes = xpAS::get($rs, '*,route');
        $rs = xpTable::load($this->table)->gets(array('route' => $routes));
        foreach ($rs as $k => $v) {
            $brr[$v['route']]['label'] = $v['route'];
            foreach ($routes as $kr => $vr) $brr[$vr]['data'][xpAS::preg_get($v['created'], '/^.{10}/ims')] += ($vr == $v['route'] ? 1 : 0);
            //$brr[$v['route']]['data'][xpAS::preg_get($v['created'], '/^.{10}/ims')] ++ ;
